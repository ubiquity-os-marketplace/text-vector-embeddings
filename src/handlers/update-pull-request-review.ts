...
export async function updatePullRequestReview(context: Context<"pull_request_review.edited">) {
  ...
  try {
    if (!pullRequest) {
      logger.error("Pull request payload missing; cannot update review summary", { reviewId: review.id });
      return;
    }
    ...
    if (config.demoFlag) {
      logger.info("Demo mode active - skipping pull request review update in database", { review: review.id, review_url: review.html_url });
      return;
    }
